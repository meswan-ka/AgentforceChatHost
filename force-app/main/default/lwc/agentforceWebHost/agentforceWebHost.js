...
import getSessionRuleAction from '@salesforce/apex/AgentforceChatHostController.getSessionRuleAction';
 ...
 */
export default class AgentforceWebHost extends LightningElement {
    ...
    _sessionRuleAction = null; // Session rule action from Agent_Session_Rule__c (null = persist by default)
    _sessionRuleLoaded = false; // Whether the session rule has been loaded
    ...
    async connectedCallback() {
        ...
        // Load session rule from Agent_Session_Rule__c
        // This determines if we should persist/restore sessions on this page
        await this._loadSessionRule();

        // Try to restore an existing session from localStorage
        // Only if the rule allows persistence
        if (this._shouldPersistSession()) {
            const storedSession = this._loadSession();
            if (storedSession && storedSession.conversationId) {
                this._restoreSession(storedSession);
            } else {
                this.generateSessionId();
            }
        } else {
            // Rule says don't persist - start fresh
            this._clearStoredSession();
            this.generateSessionId();
        }

        // Check for search integration on load (only if not restoring a session)
        if (!this.isConversationActive && this.enableSearchIntegration && !this._searchIntegrationProcessed) {
    ...
    disconnectedCallback() {
        // Save session to localStorage before component is destroyed (for page navigation)
    ...
    _checkForSearchQuery() {
        // Mark as processed at the START to prevent retries and ensure graceful handling
        // when no search query is found (component renders welcome screen normally)
        ...
        try {
            ...
            // If no query found, component renders normally (welcome screen)
        } catch (error) {
            // Silently fail - search integration is optional, component renders normally
        }
    }
    ...
    handleSSEMessage(event) {
        try {
            ...
            console.log('[WebHost] SSE RECEIVED:', {
                entryType,
            ...
            if (entryType === 'Message') {
                ...
                if (sender && (sender.role === 'Chatbot' || sender.role === 'Agent')) {
                    ...
                    if (messageText) {
                        ...
                        const htmlContent = this._markdownToHtml(messageText);
                        this.addMessage(htmlContent, 'agent', true);
                        this.messageCount++;
    ...
    async handleSendMessage() {
        ...
        if (this._screenState === 'welcome') {
            ...
            console.log('[WebHost] DOM EVENT DISPATCH: sessionstarted', {
        ...
        try {
            ...
            // Save session after sending message successfully
            this._saveSession();
        } catch (error) {
            this.isAgentTyping = false;
    ...
    /**
     * Get the localStorage key for session persistence
     */
    _getStorageKey() {
        return `agentforce_session_${this.deploymentDeveloperName}`;
    }

    /**
     * Check if session should persist based on Agent_Session_Rule__c records
     * Default behavior: persist session (no rule = persist)
     * Rules can override: Do_Not_Persist, Clear_Session
     */
    _shouldPersistSession() {
        // If a rule action says not to persist, return false
        if (this._sessionRuleAction === 'Do_Not_Persist' || this._sessionRuleAction === 'Clear_Session') {
            return false;
        }
        // Default: persist session (null or no rule = persist)
        return true;
    }

    /**
     * Get the current route from the URL
     * Handles both Aura sites (with /s/) and LWR sites (without /s/)
     */
    _getCurrentRoute() {
        const pathname = window.location.pathname;

        // Check if this is an Aura site (has /s/ in the path)
        const sIndex = pathname.indexOf('/s/');
        if (sIndex !== -1) {
            // Aura site: return everything after /s/
            return pathname.substring(sIndex + 2) || '/';
        }

        // LWR site: find the site name and return everything after it
        // The pathname typically starts with /sitename/route
        const parts = pathname.split('/').filter(p => p);
        if (parts.length > 1) {
            // Return everything after the first segment (site name)
    ...
    /**
     * Check if current site is an Aura site (has /s/ in URL)
     */
    _isAuraSite() {
    ...
    /**
     * Load the session rule for the current deployment and route
     */
    async _loadSessionRule() {
        if (this._sessionRuleLoaded || !this.deploymentDeveloperName) {
            return;
        }

        try {
            const route = this._getCurrentRoute();
            const isAura = this._isAuraSite();

            this._sessionRuleAction = await getSessionRuleAction({
                deploymentDeveloperName: this.deploymentDeveloperName,
                route: route,
                isAuraSite: isAura
            });

            this._sessionRuleLoaded = true;

            // If rule says to clear session, do it immediately
            if (this._sessionRuleAction === 'Clear_Session') {
                this._clearStoredSession();
            }
        } catch (error) {
            // If rule loading fails, default to persist (fail open)
            this._sessionRuleAction = null;
            this._sessionRuleLoaded = true;
        }
    }
     ...
     */
    _saveSession() {
        if (!this._shouldPersistSession()) return;
        ...
        const sessionData = {
            ...
            scrtUrl: this._scrtUrl,
            orgId: this._orgId,
            sessionId: this.currentSessionId,
            messages: this.messages,
            messageCount: this.messageCount,
            sessionStartTime: this.sessionStartTime,
            sessionStartTimeFormatted: this._sessionStartTimeFormatted,
            timestamp: Date.now()
        };

        try {
            localStorage.setItem(this._getStorageKey(), JSON.stringify(sessionData));
        } catch (e) {
    ...
    /**
     * Load session from localStorage if available and not stale
     */
    _loadSession() {
        if (!this._shouldPersistSession()) {
            this._clearStoredSession();
        ...
        try {
            ...
            // Check if session is stale (older than 1 hour - matches Salesforce token timeout)
     ...
     */
    async _restoreSession(storedSession) {
        // Restore state from storage
        this._conversationId = storedSession.conversationId;
        this._accessToken = storedSession.accessToken;
        this._lastEventId = storedSession.lastEventId;
        this._scrtUrl = storedSession.scrtUrl;
        this._orgId = storedSession.orgId;
        this.currentSessionId = storedSession.sessionId;
        this.messages = storedSession.messages || [];
        this.messageCount = storedSession.messageCount || 0;
        this.sessionStartTime = storedSession.sessionStartTime;
        this._sessionStartTimeFormatted = storedSession.sessionStartTimeFormatted || this._formatTime(new Date(this.sessionStartTime));
        ...
        // Reconnect SSE for real-time messages
        this.subscribeToSSE();
    ...
    publishToChannel(eventType, data = {}) {
        ...
        console.log('[WebHost] LMS DISPATCH:', {
            eventType,
            sessionId: this.currentSessionId,
            data
        });
        ...
        try {
            ...
            console.error('[WebHost] LMS DISPATCH ERROR:', error);
        }
    }
    ...
    /**
     * Convert markdown text to HTML for rendering in lightning-formatted-rich-text
     * Supports: **bold**, - bullet lists, nested lists, line breaks
     */
    _markdownToHtml(text) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            // Check for list items (-, *, •) with optional indentation
            ...
            if (listMatch) {
                ...
                // Start new list or adjust depth
                if (!inList) {
                    inList = true;
                    html += '<ul>';
                    listStack = [0];
                ...
                const processedContent = this._processInlineMarkdown(content);
                ...
                if (line.trim()) {
                    const processedLine = this._processInlineMarkdown(line);
    ...
    _processInlineMarkdown(text) {
        ...
        // Convert citation references like [1] to superscript
    ...
    endSession() {
        if (this.isConversationActive) {
            ...
            console.log('[WebHost] DOM EVENT DISPATCH: sessionended', {
                sessionId: this.currentSessionId,
                conversationId: this._conversationId,
                messageCount: this.messageCount
            });
